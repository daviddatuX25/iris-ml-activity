...
with col_viz:
    ...
    # We multiply the cm values by 30 to convert them to pixel sizes that look good on screen
    sl = sepal_length * 30
    sw = sepal_width * 30
    pl = petal_length * 30
    pw = petal_width * 30
    
    # We use raw HTML/SVG to draw ellipses. 
    # Sepals are green (arranged vertically/horizontally)
    # Petals are purple (arranged diagonally)
    svg_code = f"""
    <div style="display: flex; justify-content: center; align-items: center; height: 100%; background-color: #f0f2f6; border-radius: 10px; padding: 20px;">
